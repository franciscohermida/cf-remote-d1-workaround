...
## What fails (no workaround) ** using remote D1 in dev **

1. `pnpm db:migration:apply --local` (prisma limitation: we need to keep local db in sync to create future migration scripts)
2. `pnpm db:migration:apply --remote` (apply 0001. The 0002 migration is ignored because of the .sql\_ extension)
3. `pnpm db:seed --remote`
4. `pnpm db:snapshot` or `pnpm db:snapshot:export --remote` (first snapshot for comparison)
...
8. `pnpm db:snapshot` or `pnpm db:snapshot:export --remote` (shows failure compared to first snapshot)

## Working sequence (with workaround)

1. `pnpm db:reset --local`
2. `pnpm db:reset --remote`
3. rename `0002_change-user.sql` → `0002_change-user.sql_` (to simulate that only 0001 exists)
4. `pnpm db:migration:apply:workaround --local` (local kept current; Prisma `migration diff` needs it)
5. `pnpm db:migration:apply:workaround --remote` (npm script already supplies the DB name)
6. `pnpm db:seed --remote`
7. `pnpm db:snapshot` or `pnpm db:snapshot:export --remote`
8. rename `0002_change-user.sql_` → `0002_change-user.sql` (simulate a schema change)
9. `pnpm db:migration:apply:workaround --local` (sync local)
10. `pnpm db:migration:apply:workaround --remote` (npm script already supplies the DB name)
11. `pnpm db:snapshot` or `pnpm db:snapshot:export --remote` (confirms success)
